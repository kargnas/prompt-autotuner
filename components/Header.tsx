...
const Header: React.FC<HeaderProps> = ({ onOpenSettings, onReset }) => {
  ...
  return (
    <header className="py-2 px-3 border-b border-gray-200 bg-white z-10 flex items-center justify-between">
      ...
      <div className="flex items-center space-x-1.5 md:flex-1 md:justify-center">
        <SparklesIcon className="w-5 h-5 text-cyan-500" />
        <h1 className="text-sm sm:text-base font-bold text-transparent bg-clip-text bg-gradient-to-r from-gray-800 to-cyan-500 whitespace-nowrap">
          {t('header.title')}
        </h1>
      </div>

      {/* Buttons container: right-aligned */}
      <div className="flex items-center justify-end md:flex-1 space-x-1.5">
        <div className="flex bg-gray-100 rounded-md p-0.5 border border-gray-200">
          {SUPPORTED_LANGUAGES.map((code) => (
            <button
              ...
              className={`px-1.5 py-0.5 text-[11px] font-medium rounded ${language === code ? 'bg-white shadow-sm text-cyan-700' : 'text-gray-500 hover:text-gray-900'}`}
            >
